...
#for inspiration on the regular expressions
...
def sanitize(text):
    ...
    text = re.sub(r'[\n\t]', ' ', text)
    no_url = re.sub(r'https?:\/\/\S+', '', text)    #remove URLs of the form http[s]://facebook.com
    no_url2 = re.sub(r'www\.', '', no_url)          #remove URLs of the form www.google.com
    no_url3 = re.sub(r'^\/\S+', '', no_url2)        #remove URLs of the form /r/politics/etc
    split = no_url3.split()
    punctuated = []
    for token in split:
        token = token.lower()
